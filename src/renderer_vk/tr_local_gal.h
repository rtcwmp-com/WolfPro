...
inline void Pool_Clear(memoryPool* pool) {
	pool->firstFree = 1;
...
inline void Pool_Init(memoryPool *pool, const uint32_t itemCount, const uint32_t typeSize, uint16_t type){
	pool->poolType = type;
...
inline uint64_t ComposeHandle(uint32_t index, uint16_t generation,uint16_t type){
...
inline DecomposedHandle DecomposeHandle(uint64_t handle){
...
inline uint64_t Pool_Add(memoryPool *pool, void *rawItem){
...
inline void Pool_Remove(memoryPool *pool, uint64_t handle){
...
inline void* Pool_Get(memoryPool *pool, uint64_t handle){
	DecomposedHandle item = DecomposeHandle(handle);
